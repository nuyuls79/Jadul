...
class CategoryAdapter(private val categories: ArrayList<Category>?) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // tetap set adapter channel (dipakai sistem favorit & internal logic)
        holder.binding.chAdapter =
            ChannelAdapter(category?.channels, position, isFav)

        // klik kategori → kirim ke MainActivity untuk ubah grid utama
        holder.itemView.setOnClickListener {
            category?.let { categoryClickListener?.invoke(it) }
        }

        // hitung grid channel (dipakai layout lama & kompatibilitas)
        ...
        // margin khusus kategori pertama (favorite)
